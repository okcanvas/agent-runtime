...
async function main(): Promise<void> {
  ...
  try {
    ...
    const client = await (await fetch(`${baseUrl}/api/v1/context/entities/CLIENT/client-0042`, { headers })).json() as {
      record: { relations: Array<{ relation_type: string }> };
    };
    ...
    const checks = {
      typescript_build_dependency_closed:
        packageJson.version === "0.2.2" && packageJson.devDependencies?.typescript === "file:vendor/typescript-5.8.3.tgz" &&
      ...
      deterministic_identity: health.example_template_only === true && health.version === "0.2.2",
    ...
    const payload = {
      schema_version: "okcanvas-organization-context-example-step002-acceptance-v1",
    ...
    const output = resolve(root, "docs/evidence/EXAMPLE_ORGANIZATION_CONTEXT_STEP002R2_ACCEPTANCE.json");
