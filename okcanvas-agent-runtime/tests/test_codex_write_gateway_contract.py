...
import okcanvas_agent_runtime.adapters.openai.runtime.codex_write_gateway as gateway_module


def test_codex_write_gateway_uses_workspace_write_without_secret_inheritance(monkeypatch, tmp_path: Path) -> None:
    captured = {}
    ...
    class FakeTurnCompletedEvent:
        def __init__(self):
            ...
            self.usage = SimpleNamespace(input_tokens=40, cached_input_tokens=5, output_tokens=8)
    ...
    def fake_codex_tool(**kwargs):
        ...
        return SimpleNamespace(name="codex_engineer")
    ...
    class FakeRunner:
        ...
        async def run(cls, agent, request, *, context, max_turns, run_config):
            ...
            await captured["tool"]["on_stream"](SimpleNamespace(event=FakeTurnCompletedEvent()))
            context["codex_thread_id_writer"] = "thread-write"
    ...
    fake_agents.gen_trace_id = lambda: "trace-write"
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "must-not-be-in-child-env")
    ...
    monkeypatch.setitem(sys.modules, "agents.extensions.experimental", types.ModuleType("agents.extensions.experimental"))
    ...
    assert captured["agent"]["output_type"] is CodexWriteResult
    assert captured["agent"]["handoffs"] == []
    assert captured["model_settings"]["tool_choice"] == "required"
    assert captured["tool"]["sandbox_mode"] == "workspace-write"
    ...
    assert result.thread_id == "thread-write"
    assert result.agent_usage.total_tokens == 19
    assert result.codex_usage.input_tokens == 40
