...
from okcanvas_agent_runtime.agent.sdk.codex_write_agent import CODEX_WRITE_AGENT_ID, CODEX_WRITE_THREAD_CONTEXT_KEY, build_codex_write_agent
...
class CodexWriteGatewayRunResult:
...
class CodexWriteGateway(Protocol):
    ...
    async def run(
        ...
        *,
        ...
        if not readiness.ready:
            ...
            raise CodexWriteFailure(CodexWriteErrorCode(issue.code.value), issue.message)
        ...
        run_config = RunConfig(
            ...
            **trace_run_config_settings,
            trace_metadata={
                "run_id": run_id,
                "agent_id": CODEX_WRITE_AGENT_ID,
                "mode": "disposable-workspace-write",
        ...
        try:
            ...
            raise CodexWriteFailure(
                CodexWriteErrorCode.CODEX_RUN_FAILED,
                "OpenAI Agents SDK Codex workspace-write run failed",
                retryable=True,
                detail_type=type(exc).__name__,
            ) from exc

        try:
            output = result.final_output_as(CodexWriteResult, raise_if_incorrect_type=True)
            if not isinstance(output, CodexWriteResult):
                output = CodexWriteResult.model_validate(output)
        except (TypeError, ValidationError, ValueError) as exc:
            raise CodexWriteFailure(
                CodexWriteErrorCode.OUTPUT_CONTRACT_INVALID,
                "The Codex Agent output did not match CodexWriteResult",
                detail_type=type(exc).__name__,
            ) from exc

        thread_id = context.get(CODEX_WRITE_THREAD_CONTEXT_KEY) or journal.thread_id
