...
import { answerSummary, capabilitySummary, isCliCompatible, renderAgentChoices, renderAgents, renderAnswer, renderDebugArtifact, renderDebugPreflight, renderDebugRun, renderDetails, renderEvaluation, renderEventLine, renderEvents, renderInvocations, renderProgressEvent, renderSession, renderSessions, shortSessionId } from "./render.js";
...
export class PersistentAgentCli {
    ...
    async run() {
        try {
            ...
            if (this.agents.length === 0)
                throw new CliError("CLI_NO_COMPATIBLE_AGENT", "실행 가능한 text-only Agent가 없습니다");
    ...
    sessionEnabled(agent = this.activeAgent) {
    ...
    async selectInitialAgent(requested) {
        if (requested) {
            const found = this.agents.find((item) => item.agent_id === requested);
            if (!found)
                throw new CliError("CLI_AGENT_NOT_FOUND", `사용 가능한 Agent가 아닙니다: ${requested}`);
            return found;
        }
        ...
        while (true) {
            ...
            if (answer === null)
                throw new CliError("CLI_AGENT_SELECTION_CANCELLED", "Agent selection was cancelled");
    ...
    async activateSession(requestedSessionId) {
        ...
        if (requestedSessionId) {
            this.activeSession = await this.validateSessionForActiveAgent(await this.client.getSession(requestedSessionId));
    ...
    async validateSessionForActiveAgent(session) {
        ...
        if (session.agent_definition_id !== this.activeAgent.agent_id) {
            throw new CliError("CLI_SESSION_AGENT_MISMATCH", `Session ${session.session_id} belongs to ${session.agent_definition_id}, not ${this.activeAgent.agent_id}`);
        }
        return session;
    }
    printBanner(version) {
    ...
    async handleCommand(line) {
        ...
        switch (command) {
            ...
            case "/model":
                if (!argument)
                    console.log(`현재 모델: ${this.model ?? "server default"}`);
                else {
    ...
    async sessionsForActiveAgent() {
        if (!this.sessionEnabled())
            return [];
        return (await this.client.listSessions()).filter((session) => session.agent_definition_id === this.activeAgent.agent_id);
    }
    async showSessions() {
        if (!this.sessionEnabled()) {
            console.log("현재 Agent는 Runtime Session을 지원하지 않습니다.");
        ...
        return {
            onPreflight: (preflight, agent, model) => {
                if (this.debugEnabled)
                    console.log(renderDebugPreflight(preflight, agent, model));
            },
            onConfirmed: () => {
                if (this.debugEnabled)
                    console.log("\n[Events]");
    ...
    async executeRequest(requestText) {
        ...
        console.log("실행 준비 중...");
        const outcome = await this.client.execute(this.activeAgent, requestText, this.model, this.activeSession?.session_id, async () => {
        ...
        }, this.options.evaluationCaseId, this.executionObserver());
        if (!outcome) {
