...
export function parseArgs(argv) {
    const options = { assumeYes: false, noColor: false, debug: false };
    for (let index = 0; index < argv.length; index += 1) {
        ...
        switch (value) {
            case "--base-url":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--base-url requires a value");
                options.baseUrl = next;
                index += 1;
                break;
            case "--agent-id":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--agent-id requires a value");
                options.agentId = next;
                index += 1;
                break;
            case "--session-id":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--session-id requires a value");
                options.sessionId = next;
                index += 1;
                break;
            case "--model":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--model requires a value");
                options.model = next;
                index += 1;
                break;
            case "--evaluation-case-id":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--evaluation-case-id requires a value");
                options.evaluationCaseId = next;
                index += 1;
                break;
            case "--env-file":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--env-file requires a value");
                options.envFile = next;
                index += 1;
                break;
            case "--script":
                if (!next)
                    throw new CliError("CLI_ARGUMENT_INVALID", "--script requires a value");
...
export function loadCanonicalLocalEnvironment(pathOverride) {
    ...
    for (const [lineIndex, rawLine] of text.split(/\r?\n/u).entries()) {
        ...
        if (line.toLowerCase() === "@echo off" || line.toLowerCase().startsWith("set ")) {
            throw new CliError("CLI_ENV_FILE_INVALID", `${path}:${lineIndex + 1}: use canonical NAME=value syntax; .env.local.cmd is not supported`);
        }
...
export function runtimeConfig(options) {
    ...
    const baseUrl = validateLoopbackBaseUrl(options.baseUrl ?? process.env.OKCANVAS_CONTROL_BASE_URL ?? `http://${hostForUrl}:${port}`);
