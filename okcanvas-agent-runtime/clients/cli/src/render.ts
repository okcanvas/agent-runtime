...
function isolatedCapabilityCount(agent: AgentDefinition): number {
...
function safeReadOnlyTool(agent: AgentDefinition): boolean {
  if (agent.tools.length !== 1 || agent.tool_capabilities.length !== 1 || agent.session_mode !== "disabled") return false;
...
export function isCliCompatible(agent: AgentDefinition): boolean {
  if (!new Set(["disabled", "sqlite-v1"]).has(agent.session_mode)) return false;
  if (agent.workspace_access !== "none" || agent.mcp_servers.length > 0 || agent.guardrails.length > 0) return false;
  if (isolatedCapabilityCount(agent) > 1) return false;
  if (agent.tools.length > 0) return safeReadOnlyTool(agent);
...
export function capabilityKind(agent: AgentDefinition): string {
  if (agent.tools.length === 1) return `read-only Tool ${agent.tools[0]}`;
  if (agent.handoffs.length === 1) return `Handoff → ${agent.handoffs[0]}`;
  if (agent.agent_tools.length === 1) return `Sub Agent → ${agent.agent_tools[0]}`;
  if (agent.orchestration_children.length === 2) return `병렬 Specialist 2개 → ${agent.orchestration_children.join(", ")}`;
  return "text-only";
}

export function capabilitySummary(agent: AgentDefinition): string[] {
  ...
  if (agent.session_mode === "sqlite-v1") capabilities.push("Runtime Session 대화 기억 사용 · 같은 Session의 이전 Turn 참조 가능");
  else capabilities.push("서버 대화 기억 없음 · 각 요청은 독립 실행");
  if (agent.tools.length === 1) capabilities.push(`승인 없는 read-only Function Tool 사용 가능: ${agent.tools[0]}`);
  else if (agent.handoffs.length === 1) capabilities.push(`전문 Agent로 한 번 Handoff 가능: ${agent.handoffs[0]}`);
  else if (agent.agent_tools.length === 1) capabilities.push(`전문 Sub Agent를 Tool로 한 번 호출 가능: ${agent.agent_tools[0]}`);
  else if (agent.orchestration_children.length === 2) capabilities.push(`고정 Specialist 2개를 병렬 실행하고 선언 순서로 집계: ${agent.orchestration_children.join(", ")}`);
  else capabilities.push("현재는 입력한 대화 텍스트만 분석 가능");
  return capabilities;
}

function sessionModeLabel(agent: AgentDefinition): string {
  return agent.session_mode === "sqlite-v1" ? "대화 기억" : "독립 요청";
}

function agentLine(agent: AgentDefinition): string {
  return `${agent.agent_id} · ${agent.name} · ${sessionModeLabel(agent)} · ${capabilityKind(agent)}`;
}

export function renderAgents(agents: AgentDefinition[], activeId: string): string {
  return agents.map((agent) => `${agent.agent_id === activeId ? "*" : " "} ${agentLine(agent)}`).join("\n");
}

export function renderAgentChoices(agents: AgentDefinition[]): string {
  return agents.map((agent, index) => `  ${index + 1}. ${agentLine(agent)}`).join("\n");
}

export function renderSession(session: ProductSession): string {
  return [
...
export function renderSessions(sessions: ProductSession[], activeSessionId?: string): string {
  if (sessions.length === 0) return "현재 Agent의 Session이 없습니다.";
...
export function renderAnswer(content: JsonObject): string {
...
export function answerSummary(content: JsonObject): string {
  return typeof content.summary === "string" && content.summary.trim()
    ? content.summary.trim()
    : JSON.stringify(content);
}

function invocationDepth(item: JsonObject): number {
  const value = Number(item.depth ?? 0);
  return Number.isFinite(value) && value >= 0 ? value : 0;
}

export function renderInvocations(invocations: JsonObject[]): string {
  if (invocations.length === 0) return "Invocation이 없습니다.";
...
export function renderDetails(outcome: RunOutcome): string {
  return [
...
export function renderProgressEvent(event: RunEvent): string | undefined {
  const payload = event.payload ?? {};
  if (event.event_type === "tool.started") return `  ↳ Tool ${present(payload.tool_name)} 실행`;
  if (event.event_type === "tool.completed") return `  ✓ Tool ${present(payload.tool_name)} 완료`;
  if (event.event_type === "agent.handoff") return `  ↳ Handoff ${present(payload.from_agent_id)} → ${present(payload.to_agent_id)}`;
  if (event.event_type === "agent.tool.started") return `  ↳ Sub Agent ${present(payload.to_agent_id)} 호출`;
  if (event.event_type === "agent.tool.completed") return `  ✓ Sub Agent ${present(payload.to_agent_id)} 완료`;
  if (event.event_type === "orchestration.child.started") return `  ↳ Specialist #${present(payload.ordinal)} ${present(payload.agent_id)} 시작`;
  if (event.event_type === "orchestration.child.completed") return `  ✓ Specialist #${present(payload.ordinal)} ${present(payload.agent_id)} 완료`;
  if (event.event_type === "orchestration.child.failed") return `  ✗ Specialist #${present(payload.ordinal)} ${present(payload.agent_id)} 실패`;
  if (event.event_type === "orchestration.child.cancelled") return `  - Specialist #${present(payload.ordinal)} ${present(payload.agent_id)} 취소`;
  return undefined;
}

export function renderEvents(events: RunEvent[]): string {
  return events.map(renderEventLine).join("\n");
}

export function renderDebugPreflight(preflight: JsonObject, agent: AgentDefinition, model: string | undefined): string {
  return [
    "",
    "[Preflight]",
...
export function renderDebugRun(outcome: RunOutcome): string {
  return [
    "",
    "[Run]",
...
export function renderDebugArtifact(artifact: Artifact): string {
  return [
    "",
...
export function renderEvaluation(evaluation?: JsonObject): string {
  ...
  if (checks && typeof checks === "object" && !Array.isArray(checks)) {
    for (const [name, value] of Object.entries(checks as JsonObject)) {
