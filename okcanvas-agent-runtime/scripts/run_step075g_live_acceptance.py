...
ADMIN_KEY = "step075g-live-admin-key-123456789"
SUBMITTER_KEY = "step075g-live-submitter-key-123456"
SERVICE_TOKEN = "step075g-live-service-token-123456789"
...
def _default_output() -> Path:
    ...
    return ROOT / "docs/evidence/step075g-live" / f"{stamp}-{uuid.uuid4().hex[:8]}" / "acceptance-summary.json"
...
def _wait_terminal(client: TestClient, run_id: str, timeout: float = 300.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if payload.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return payload
        time.sleep(0.25)
    raise RuntimeError("STEP075G live Run did not become terminal within 300 seconds")
...
def run_acceptance(output: Path) -> int:
    ...
    if settings is None:
        payload = {
            "schema_version": "okcanvas-step075g-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "state": "FAILED",
            "started_at": _utc_now(),
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP075G", output=output) as workspace:
            ...
            with TestClient(app) as client:
                ...
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": f"step075g-live-{uuid.uuid4().hex}",
                    },
                )
            ...
            checks = {
                ...
                and runtime_info.next_selected_step == "UNSELECTED_PENDING_STEP075G_WINDOWS_LIVE_ACCEPTANCE",
            }
