...
STEP = "STEP088R1_ORGANIZATION_CONTEXT_BOUNDED_RESPONSE_AND_SAFE_MCP_DIAGNOSTIC_CLOSURE"
VERSION = "2.68.1"
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP088R1_DETERMINISTIC_ACCEPTANCE.json"
PARENT_PATH = ROOT / "docs/evidence/STEP088_DETERMINISTIC_ACCEPTANCE.json"
...
def run(output: Path) -> int:
    ...
    print("[STEP088R1] execution-plane", file=sys.stderr, flush=True)
    execution = validate_execution_plane()
    print("[STEP088R1] distribution", file=sys.stderr, flush=True)
    distribution = validate_distribution()
    print("[STEP088R1] launcher-registry", file=sys.stderr, flush=True)
    registry = validate_registry()
    print("[STEP088R1] architecture", file=sys.stderr, flush=True)
    ...
    print("[STEP088R1] focused-regression", file=sys.stderr, flush=True)
    ...
    print("[STEP088R1] compileall", file=sys.stderr, flush=True)
    ...
    print("[STEP088R1] assemble", file=sys.stderr, flush=True)
    checks = {
        "identity_exact": CURRENT_STEP == info.step == STEP
        and PROJECT_VERSION == info.version == VERSION,
        "step088_parent_retained": parent.get("state") == "PASSED"
        ...
        "bounded_response_contract_exact": info.main_assistant_organization_context_max_mcp_result_chars == 32000
        and info.main_assistant_organization_context_bounded_response_contract_implemented is True,
        "safe_mcp_failure_diagnostics_exact": info.main_assistant_organization_context_safe_mcp_failure_diagnostics_implemented is True
        and "MCP_RESULT_LIMIT_EXCEEDED" in (ROOT / "okcanvas_agent_runtime/adapters/openai/generic_gateway.py").read_text(encoding="utf-8")
        and "tool.failed" in (ROOT / "okcanvas_agent_runtime/adapters/openai/generic_gateway.py").read_text(encoding="utf-8"),
        ...
        "current_windows_acceptance_not_claimed": info.step088r1_windows_deterministic_accepted
        is False,
    }
    payload = {
        "schema_version": "okcanvas-step088r1-deterministic-acceptance-v1",
        "step": STEP,
        "version": VERSION,
    ...
    print("[STEP088R1] write", file=sys.stderr, flush=True)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print("[STEP088R1] emit", file=sys.stderr, flush=True)
