...
ADMIN_KEY = "step053-local-admin-key"
SUBMITTER_KEY = "step053-run-submitter-key"
...
MODEL = "deterministic-step053-model"
REQUEST = "Return a safe final answer while the SDK result contains private reasoning evidence."
HIDDEN_API_KEY = "step053-hidden-api-key"
REASONING_SUMMARY_SENTINEL = "STEP053-PRIVATE-REASONING-SUMMARY"
REASONING_CONTENT_SENTINEL = "STEP053-PRIVATE-REASONING-CONTENT"
REASONING_ENCRYPTED_SENTINEL = "STEP053-ENCRYPTED-REASONING"
REASONING_PROVIDER_SENTINEL = "STEP053-PROVIDER-DATA"
REASONING_ITEM_ID = "rs_step053_private"
...
def _install_fakes():
    ...
    output = CodingAgentResult(
        ...
        summary="Safe output completed while reasoning evidence remained minimized.",
        findings=[],
        unverified=[],
    )
    ...
    class FakeResult:
        def __init__(self, *, agent, hooks):
            ...
            self.last_response_id = "resp-step053"
        ...
        async def stream_events(self):
            ...
            response = SimpleNamespace(
                response_id="resp-step053",
                request_id="req-step053",
    ...
    class FakeRunner:
        ...
        async def run(cls, *args, **kwargs):
            ...
            raise AssertionError("STEP053 must use Runner.run_streamed")
    ...
    fake_agents.gen_trace_id = lambda: "trace-step053"
...
def _wait_terminal(client: TestClient, run_id: str) -> dict[str, object]:
    ...
    while time.monotonic() < deadline:
        ...
        if body.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return body
        time.sleep(0.02)
    raise RuntimeError("STEP053 Run did not become terminal")
...
def run_acceptance(output: Path) -> int:
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP053", output=output) as workspace:
            ...
            with TestClient(app) as client:
                denied = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": "step053-denied-route-0001",
                    },
                )
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "input": REQUEST,
                        "model": MODEL,
                        "idempotency_key": "step053-success-0001",
                    },
                )
                ...
                evaluation_response = client.post(
                    ...
                    json={"case_id": "immutable-reasoning-evidence-minimization-v1"},
                )
                ...
                drift_preflight_response = client.post(
                    ...
                    json={
                        "agent_definition_id": "coding-agent",
                        "input": "Reasoning evidence policy drift must block confirmation before Product state.",
                        "model": MODEL,
                        "idempotency_key": "step053-policy-drift-0001",
                    },
                )
                drift_preflight_response.raise_for_status()
                drift_preflight = drift_preflight_response.json()
                policy_path = project_root / "specs/runtime/reasoning-evidence-policy.json"
            ...
            policy = ReasoningEvidencePolicyCatalog(project_root).resolve()
            ...
            checks = {
                ...
                "exact_reasoning_evidence_policy_bound": (
                    binding.reasoning_evidence_policy["policy_sha256"] == policy.policy_sha256
                    and len(binding.reasoning_evidence_runtime_sha256) == 64
                ),
                "reasoning_summary_request_explicitly_disabled": (
                    captured["run_config_reasoning"] == [None]
                    and policy.reasoning_summary_requested is False
                ),
                "additional_reasoning_response_includes_absent": (
                    captured["run_config_response_include"] == [[]]
                    and policy.response_include == ()
                ),
                "reasoning_content_persistence_forbidden": (
                    policy.persist_reasoning_content is False
                    and policy.persist_reasoning_summary is False
                    and policy.persist_reasoning_item_ids is False
                    and policy.persist_reasoning_provider_data is False
                ),
                "reasoning_token_count_only_allowed": policy.persist_reasoning_token_count is True,
                ...
                "reasoning_item_observed_as_count_only": len(completed_events) == 1
                and completed_events[0]["payload"].get("reasoning_item_count") == 1
                and completed_events[0]["payload"].get("reasoning_content_persisted") is False
                and completed_events[0]["payload"].get("reasoning_summary_persisted") is False
                and completed_events[0]["payload"].get("reasoning_item_ids_persisted") is False
                and completed_events[0]["payload"].get("reasoning_provider_data_persisted") is False,
                "model_event_policy_metadata_exact": len(model_events) == 1
                and model_events[0]["payload"].get("reasoning_evidence_policy_id")
                == "local-openai-reasoning-evidence-minimization-v1"
                and model_events[0]["payload"].get("reasoning_summary_requested") is False
                and model_events[0]["payload"].get("reasoning_response_include_count") == 0
                and model_events[0]["payload"].get("reasoning_token_count_persisted") is True,
                "reasoning_token_count_persisted_without_content": len(run_completed) == 1
                and run_completed[0]["payload"].get("usage", {}).get("reasoning_tokens") == 11,
                "reasoning_raw_fields_absent_from_events": all(
                    value not in event_text for value in sensitive_values
                ),
                "reasoning_raw_fields_absent_from_product_and_evaluation_db": all(
                    value not in product_text and value not in evaluation_text
                    for value in sensitive_values
                ),
                "reasoning_raw_fields_absent_from_artifact": all(
                    value not in artifact_text for value in sensitive_values
                ),
                ...
                "reasoning_policy_drift_blocked_confirmation": drift_confirmation.status_code == 409,
                "reasoning_policy_drift_created_no_second_task_or_run": final_counts["tasks"] == 1
            ...
            report = {
                "schema_version": "okcanvas-step053-acceptance-v1",
                "state": "PASSED" if all(checks.values()) else "FAILED",
                "checks": checks,
                "reasoning_policy": {
                    **policy.to_binding_dict(),
                    "runtime_binding_sha256": binding.runtime_binding_sha256,
                    "runtime_source_sha256": binding.reasoning_evidence_runtime_sha256,
                    "observed_reasoning_items": (
                        completed_events[0]["payload"].get("reasoning_item_count")
                        if completed_events else None
                    ),
                    "observed_reasoning_tokens": (
                        run_completed[0]["payload"].get("usage", {}).get("reasoning_tokens")
                        if run_completed else None
                    ),
                },
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "docs/evidence/STEP053_ACCEPTANCE.json",
    )
