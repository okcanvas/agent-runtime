...
VERSION = "2.64.0"
RECORD = ROOT / "docs/evidence/STEP084_CONSTITUTION_COMPLIANCE.json"
...
def validate(path: Path = RECORD) -> dict[str, Any]:
    ...
    architecture = _load(ROOT / "docs/evidence/STEP084_ARCHITECTURE_VALIDATION.json")
    organization_context = _load(ROOT / "docs/evidence/STEP084_ORGANIZATION_CONTEXT_VALIDATION.json")
    python_regression = _load(ROOT / "docs/evidence/STEP084_PYTHON_REGRESSION.json")
    non_python = _load(ROOT / "docs/evidence/STEP084_NON_PYTHON_VALIDATION.json")
    installation = _load(ROOT / "docs/evidence/STEP084_INSTALLATION_VALIDATION.json")
    fresh = _load(ROOT / "docs/evidence/STEP084_FRESH_ZIP_VALIDATION.json")
    acceptance = _load(ROOT / "docs/evidence/STEP084_ACCEPTANCE.json")
    ...
    checks = {
        ...
        "organization_context_passed": organization_context.get("state") == "PASSED" and organization_context.get("passed_checks") == organization_context.get("total_checks") == 20,
        "full_python_passed": python_regression.get("state") == "PASSED" and python_regression.get("passed_tests") == python_regression.get("total_tests") and python_regression.get("test_file_count") == len(list((ROOT / "tests").glob("test_*.py"))),
        ...
        "fresh_zip_passed": fresh.get("state") == "PASSED" and not fresh.get("candidate", {}).get("forbidden_entries"),
        "acceptance_passed": acceptance.get("state") == "PASSED" and acceptance.get("passed_checks") == acceptance.get("total_checks") == 12,
        "issues_complete": all(f"OR-ISSUE-{number:03d}" in issue_registry for number in range(17, 75)),
        "windows_product_rerun_separate_from_constitution_gate": record.get("validation_environment", {}).get("step084_windows_deterministic") == "PENDING_EXTERNAL_PRODUCT_PROMOTION_GATE",
    }
    return {
        "schema_version": "okcanvas-step084-compliance-validation-v1",
        "step": STEP,
        "version": VERSION,
