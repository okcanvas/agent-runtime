...
STEP = "STEP089_ORGANIZATION_CONTEXT_SHORT_EXPRESSION_ROUTING_AND_PACKAGE_ACCEPTANCE_CLOSURE"
VERSION = "2.69.0"
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP089_DETERMINISTIC_ACCEPTANCE.json"
PARENT_PATH = ROOT / "docs/evidence/STEP088R1_DETERMINISTIC_ACCEPTANCE.json"
EXPECTED_PACKAGE_NAME = (
    "okcanvas-agent-runtime-step089-organization-context-short-expression-routing-and-"
    "package-acceptance-closure.zip"
)
...
def run(output: Path) -> int:
    ...
    print("[STEP089] execution-plane", file=sys.stderr, flush=True)
    execution = validate_execution_plane()
    print("[STEP089] distribution", file=sys.stderr, flush=True)
    distribution = validate_distribution()
    print("[STEP089] launcher-registry", file=sys.stderr, flush=True)
    registry = validate_registry()
    print("[STEP089] architecture", file=sys.stderr, flush=True)
    ...
    print("[STEP089] focused-regression", file=sys.stderr, flush=True)
    ...
    print("[STEP089] compileall", file=sys.stderr, flush=True)
    ...
    checks = {
        ...
        "step088r1_parent_retained": parent.get("state") == "PASSED"
        ...
        "current_windows_acceptance_not_claimed": info.step089_windows_deterministic_accepted
        is False,
    }
    payload = {
        "schema_version": "okcanvas-step089-deterministic-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_SHORT_EXPRESSION_ROUTING_AND_PACKAGE_GATE",
        ...
        "limitations": {
            ...
            "windows_step089_executed": False,
        },
    }
