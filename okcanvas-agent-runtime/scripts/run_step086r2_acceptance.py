...
from scripts.validate_step086r2_connector_contract import validate as validate_connector_contract

STEP = "STEP086R2_DELEGATED_ROLE_HEADER_AND_EXTERNAL_CONNECTOR_CONTRACT_CLOSURE"
VERSION = "2.66.2"
OUTPUT_DEFAULT = ROOT / "docs/evidence/step086r2-local/STEP086R2_ACCEPTANCE.json"
WINDOWS_PARENT = ROOT / "docs/evidence/STEP086R1_WINDOWS_DETERMINISTIC_ACCEPTANCE.json"
...
def run(output: Path) -> int:
    started = _now()
    info = RuntimeInfo()
    connector = validate_connector_contract()
    ...
    checks = {
        ...
        "step086r1_windows_parent_accepted": windows_parent.get("state") == "PASSED"
        and windows_parent.get("passed_checks") == windows_parent.get("total_checks") == 13,
        "connector_contract_validation_passed": connector.get("state") == "PASSED"
        and connector.get("passed_checks") == connector.get("total_checks") == 11,
        "delegated_roles_transmitted": info.delegated_mcp_roles_transmitted is True
        and info.delegated_mcp_roles_header == "X-OKCanvas-Roles",
        "credential_reference_not_transmitted": info.delegated_mcp_credential_reference_transmitted is False,
        "external_connector_path_exact": info.groupware_read_external_connector_project_path
        == "okcanvas-connectors/groupware-mcp-server",
        "optional_example_not_runtime_dependency": info.groupware_read_api_fake_example_required is False
        and info.groupware_read_api_fake_example_is_mcp_server is False,
        ...
        and registry.get("current_step") == STEP and registry.get("current_record_count") == 2,
        "focused_regression_passed": focused_ok,
        "compileall_passed": compile_ok,
        "actual_groupware_live_not_claimed": info.groupware_read_mcp_provider_live_verified is False,
        "next_step_unselected": info.next_selected_step == "UNSELECTED_PENDING_USER_SELECTION",
    }
    payload = {
        "schema_version": "okcanvas-step086r2-acceptance-v1",
        ...
        "step086r1_windows_parent": windows_parent,
