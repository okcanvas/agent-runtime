...
ADMIN_KEY = "step075a-live-admin-key-123456789"
SUBMITTER_KEY = "step075a-live-submitter-key-123456"
SERVICE_TOKEN = "step075a-live-service-token-123456789"
...
def _default_output() -> Path:
    ...
    return ROOT / "docs/evidence/step075a-live" / f"{stamp}-{uuid.uuid4().hex[:8]}" / "acceptance-summary.json"
...
def _wait_terminal(client: TestClient, run_id: str, timeout: float = 300.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if payload.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return payload
        time.sleep(0.25)
    raise RuntimeError("STEP075A live Run did not become terminal within 300 seconds")
...
def run_acceptance(output: Path) -> int:
    ...
    if settings is None:
        payload = {
            "schema_version": "okcanvas-step075a-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "state": "FAILED",
            "started_at": _utc_now(),
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP075A", output=output) as workspace:
            ...
            with TestClient(app) as client:
                ...
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": f"step075a-live-{uuid.uuid4().hex}",
                    },
                )
