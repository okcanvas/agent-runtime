...
def build(
    *,
    ...
    evidence = {
        ...
        "multi_mcp": _load(multi_mcp_path),
    ...
    with zipfile.ZipFile(candidate_zip) as archive:
        ...
        forbidden = sorted(
            ...
            or "/docs/evidence/step085-local/" in name
        )
    ...
    checks = {
        ...
        "multi_mcp_passed": evidence["multi_mcp"].get("state") == "PASSED"
        and evidence["multi_mcp"].get("passed_checks") == evidence["multi_mcp"].get("total_checks") == 22,
    ...
    payload = {
        "schema_version": "okcanvas-step085-fresh-zip-validation-v1",
        "step": STEP,
        "version": VERSION,
...
def main() -> int:
    ...
    parser.add_argument("--multi-mcp", type=Path, required=True)
    ...
    payload = build(
        ...
        multi_mcp_path=args.multi_mcp.resolve(),
