...
VERSION = "2.65.0"
RECORD = ROOT / "docs/evidence/STEP085_CONSTITUTION_COMPLIANCE.json"
...
def validate(path: Path = RECORD) -> dict[str, Any]:
    ...
    architecture = _load(ROOT / "docs/evidence/STEP085_ARCHITECTURE_VALIDATION.json")
    multi_mcp = _load(ROOT / "docs/evidence/STEP085_MULTI_MCP_DELEGATED_IDENTITY_VALIDATION.json")
    python_regression = _load(ROOT / "docs/evidence/STEP085_PYTHON_REGRESSION.json")
    non_python = _load(ROOT / "docs/evidence/STEP085_NON_PYTHON_VALIDATION.json")
    installation = _load(ROOT / "docs/evidence/STEP085_INSTALLATION_VALIDATION.json")
    fresh = _load(ROOT / "docs/evidence/STEP085_FRESH_ZIP_VALIDATION.json")
    acceptance = _load(ROOT / "docs/evidence/STEP085_ACCEPTANCE.json")
    ...
    checks = {
        ...
        "multi_mcp_passed": multi_mcp.get("state") == "PASSED" and multi_mcp.get("passed_checks") == multi_mcp.get("total_checks") == 22,
        ...
        "issues_complete": all(f"OR-ISSUE-{number:03d}" in issue_registry for number in range(17, 83)),
        "windows_product_rerun_separate_from_constitution_gate": record.get("validation_environment", {}).get("step085_windows_deterministic") == "PENDING_EXTERNAL_PRODUCT_PROMOTION_GATE",
    }
    return {
        "schema_version": "okcanvas-step085-compliance-validation-v1",
        "step": STEP,
        "version": VERSION,
