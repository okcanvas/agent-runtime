...
ADMIN_KEY = "step029-acceptance-admin-key"
SUBMITTER_KEY = "step029-acceptance-submitter-key"
PAYLOAD_KEY = base64.urlsafe_b64encode(bytes(range(32))).decode("ascii")
SOURCE_TOKEN = "step029-loopback-source-token-sentinel"
HEADERS = {
...
CASES: dict[str, dict[str, object]] = {
    "string-safety-stock": {
        "snapshot_id": "string-safety-stock",
        "safety_stock_units": "2",
        "items": [
            {"sku": "strict-a", "available_units": 4, "forecast_units": 5, "inbound_units": 0}
        ],
    },
    "boolean-available": {
        "snapshot_id": "boolean-available",
        "safety_stock_units": 2,
        "items": [
            {"sku": "strict-b", "available_units": True, "forecast_units": 5, "inbound_units": 0}
        ],
    },
    "float-forecast": {
        "snapshot_id": "float-forecast",
        "safety_stock_units": 2,
        "items": [
            {"sku": "strict-c", "available_units": 4, "forecast_units": 5.0, "inbound_units": 0}
        ],
    },
    "string-inbound": {
        "snapshot_id": "string-inbound",
        "safety_stock_units": 2,
        "items": [
            {"sku": "strict-d", "available_units": 4, "forecast_units": 5, "inbound_units": "0"}
        ],
    },
}
...
class FailIfCalledGateway:
    ...
    async def run(self, **_kwargs):
        self.calls += 1
        raise AssertionError("STEP029 scalar-type rejection must not enter the model gateway")


class ScalarCoercionSource:
    def __init__(self) -> None:
        self.read_count = 0
        self.write_count = 0
    ...
    def __enter__(self) -> "ScalarCoercionSource":
        owner = self
...
def run(output: Path) -> dict[str, object]:
    ...
    with AcceptanceWorkspace(step_id="STEP029", output=output) as workspace:
        ...
        with ScalarCoercionSource() as source:
            app = create_app(
                project_root=ROOT,
                ...
                for index, case_id in enumerate(CASES, start=1):
                    ...
                    response = client.post(
                        ...
                        json={
                            ...
                            "model": "step029-acceptance-model",
                            "idempotency_key": f"step029-strict-scalar-{index:02d}-key",
                        },
                    )
                    body = response.json()
        ...
        checks = {
            "four_scalar_coercion_cases_executed": len(case_results) == 4,
            ...
            "all_integer_fields_strictly_rejected": {item["case_id"] for item in case_results}
            == set(CASES),
            "source_read_once_per_case": reads_by_key == {key: 1 for key in sorted(CASES)},
            "source_total_reads_exact": source_read_count == 4,
            "requested_paths_exact": requested_paths == list(CASES),
            ...
            "invalid_source_snapshots_not_in_sqlite": all(
                case_id.encode("utf-8") not in database_bytes for case_id in CASES
            ),
            "references_unchanged": references_before == references_after,
        }
        payload: dict[str, object] = {
            "schema_version": "okcanvas-step029-acceptance-v1",
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "docs" / "evidence" / "STEP029_ACCEPTANCE.json",
    )
    return parser
