...
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP075B_ACCEPTANCE.json"
STEP = "STEP075B_WINDOWS_DOCKER_COMMAND_OPERATION_EVIDENCE"
VERSION = "2.55.2"
...
def run(output: Path) -> int:
    ...
    step075a_failure = _load_json(ROOT / "docs/evidence/STEP075A_WINDOWS_LIVE_ACCEPTANCE_FAILURE_SUMMARY.json")
    ...
    deterministic_launcher = (ROOT / "sh_run_step075b_acceptance.cmd").read_text(encoding="utf-8")
    live_launcher = (ROOT / "sh_run_step075b_live_acceptance.cmd").read_text(encoding="utf-8")
    ...
    compile_ok, compile_output = run_command(
        [
            ...
            "-q",
            "src",
            "scripts/run_step075b_acceptance.py",
            "scripts/run_step075b_live_acceptance.py",
        ],
        ROOT,
    )
    ...
    required_docs = (
        ROOT / "docs/plans/STEP075B_WINDOWS_DOCKER_COMMAND_OPERATION_EVIDENCE.md",
        ROOT / "docs/reference/STEP075B_WINDOWS_DOCKER_COMMAND_OPERATION_EVIDENCE_CODE_AUDIT.md",
        ROOT / "docs/evidence/STEP075_WINDOWS_LIVE_ACCEPTANCE_FAILURE_SUMMARY.json",
        ROOT / "docs/evidence/STEP075A_WINDOWS_LIVE_ACCEPTANCE_FAILURE_SUMMARY.json",
        ROOT / "docs/issues/ISSUE_REGISTRY.md",
        ROOT / "docs/issues/OR-ISSUE-001-STEP075-WINDOWS-DOCKER-TMPFS-NORMALIZATION.md",
        ROOT / "docs/issues/OR-ISSUE-002-STEP075A-WINDOWS-DOCKER-COMMAND-OPERATION-EVIDENCE.md",
    ...
    checks = {
        ...
        "step075a_windows_live_failure_recorded_exact": (
            step075a_failure.get("state") == "FAILED"
            and step075a_failure.get("deterministic_acceptance") == {"passed_checks": 31, "total_checks": 31}
            and step075a_failure.get("live_acceptance") == {"passed_checks": 13, "total_checks": 29}
            and step075a_failure.get("model_calls") == 1
            and step075a_failure.get("terminal_status") == "FAILED"
            and step075a_failure.get("product_database_relative_path") == "databases/product.sqlite3"
            and step075a_failure.get("tool_failure", {}).get("code") == "DOCKER_COMMAND_FAILED"
            and step075a_failure.get("tool_failure", {}).get("operation") is None
        ),
        ...
        "bounded_sandbox_tool_failure_evidence_present": (
            ...
            and '"operation": exc.operation' in gateway_source
            and '"return_code": exc.return_code' in gateway_source
            and '"stderr_category": exc.stderr_category' in gateway_source
            and '"cleanup_attempted": exc.cleanup_attempted' in gateway_source
            and '"cleanup_completed": exc.cleanup_completed' in gateway_source
            and '"orphan_count": exc.orphan_count' in gateway_source
            and 'payload_schema_version="okcanvas-function-tool-failed-v1"' in gateway_source
            and '"raw_arguments_persisted": False' in gateway_source
            and '"raw_output_persisted": False' in gateway_source
            and '"raw_message_persisted": False' in gateway_source
        ),
        "docker_operation_vocabulary_and_stderr_classification_present": (
            "docker_operation_name" in product_source
            and "docker_stderr_category" in product_source
            and '"container.copy_snapshot"' in product_source
            and 'return f"container.exec.{executable}"' in product_source
            and 'if executable in {"find", "cat", "grep", "tail"}' in product_source
            and 'return "UNKNOWN"' in product_source
        ),
        "primary_failure_preserved_through_cleanup": (
            "failure: SandboxDockerError | None" in snapshot_source
            and "raise failure.attach_cleanup" in snapshot_source
            and "cleanup_completed=(not cleanup_attempted or deleted)" in snapshot_source
        ),
        "step075_runtime_flags_exact": (
            ...
            and info.product_owned_readonly_sandbox_command_operation_evidence_implemented is True
            and info.next_selected_step == "UNSELECTED_PENDING_STEP075B_WINDOWS_LIVE_RERUN"
        ),
        ...
        "service_metadata_and_policy_select_step075_gate": (
            service_policy.get("next_selected_step") == "UNSELECTED_PENDING_STEP075B_WINDOWS_LIVE_RERUN"
        ...
        "windows_launchers_use_isolation_and_data_loader": (
            "python_bytecode_isolation.py scripts\\run_step075b_acceptance.py" in deterministic_launcher
            and "python_bytecode_isolation.py scripts\\windows_entrypoint.py readonly-sandbox-workspace-operation-live-acceptance" in live_launcher
            and "readonly-sandbox-workspace-operation-live-acceptance" in windows_entrypoint
            and "run_step075b_live_acceptance.py" in windows_entrypoint
        ),
        "live_evidence_and_local_secrets_are_packaging_ignored": (
            "docs/evidence/step075b-live/" in (ROOT / ".gitignore").read_text(encoding="utf-8")
            and ".env.local" in package_source
            and "step075b-live" in package_source
        ),
        "step075b_documents_present": all(path.is_file() for path in required_docs),
        "focused_step075b_tests_pass": focused_ok,
        ...
        "source_package_default_is_step075b": (
            "step075b-windows-docker-command-operation-evidence" in package_source
            and "STEP075B_WINDOWS_DOCKER_COMMAND_OPERATION_EVIDENCE" in package_source
        ),
    }
    payload = {
        "schema_version": "okcanvas-step075b-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        ...
        "model_calls": 0,
        "live_launcher": "sh_run_step075b_live_acceptance.cmd",
