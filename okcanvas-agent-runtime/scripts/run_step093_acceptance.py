...
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP093R1_DETERMINISTIC_ACCEPTANCE.json"
...
def run(output: Path, *, emit_stdout: bool = True) -> int:
    ...
    checks = {
        ...
        "runtime_package_metadata_matches_baseline": 'version = "2.77.1"' in runtime_pyproject,
        ...
        "launcher_registry_current_exact": registry.get("state") == "PASSED" and registry.get("current_step") == STEP and registry.get("current_step_token") == "093" and registry.get("current_record_count") == 2,
        ...
        "package_identity_exact": PACKAGE_STEP == STEP and DEFAULT_OUTPUT.name == "okcanvas-agent-runtime-step093r1-relation-route-protocol-and-live-false-positive-closure.zip",
    }
    payload = {
        "schema_version": "okcanvas-step093r1-deterministic-acceptance-v1",
        "step": STEP, "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_STEP093R1_RELATION_ROUTE_PROTOCOL_AND_LIVE_FALSE_POSITIVE_CLOSURE",
