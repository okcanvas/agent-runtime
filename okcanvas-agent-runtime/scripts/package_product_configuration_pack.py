...
DEFAULT_OUTPUT=ROOT.parent/'okcanvas-agent-runtime-product-configuration-pack-2.70.0.zip'
ARCHIVE_ROOT=Path('okcanvas-agent-runtime-config')
...
def package(output: Path=DEFAULT_OUTPUT) -> dict[str, object]:
    ...
    with zipfile.ZipFile(output,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=9) as zf:
        for path in sorted((p for p in (ROOT/'specs').rglob('*') if _include(p)),key=lambda p:p.relative_to(ROOT).as_posix()):
            rel=path.relative_to(ROOT); entries.append(rel.as_posix())
            info=zipfile.ZipInfo((ARCHIVE_ROOT/rel).as_posix(),date_time=(2026,8,6,0,0,0))
