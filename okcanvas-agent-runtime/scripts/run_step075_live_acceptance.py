...
ADMIN_KEY = "step075-live-admin-key-123456789"
SUBMITTER_KEY = "step075-live-submitter-key-123456"
SERVICE_TOKEN = "step075-live-service-token-123456789"
...
def _default_output() -> Path:
    ...
    return ROOT / "docs/evidence/step075-live" / f"{stamp}-{uuid.uuid4().hex[:8]}" / "acceptance-summary.json"
    ...
    return json.dumps(
        {
            ...
            "tokens": [
                {
                    "token_id": "step075-live-client",
                    "token_sha256": _sha(SERVICE_TOKEN),
                    "tenant_id": "step075-tenant",
                    "principal_id": "step075-reviewer",
                    "roles": ["agent-user"],
                }
            ],
        },
...
def _wait_terminal(client: TestClient, run_id: str, timeout: float = 300.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if payload.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return payload
        time.sleep(0.25)
    raise RuntimeError("STEP075 live Run did not become terminal within 300 seconds")
...
def run_acceptance(output: Path) -> int:
    ...
    if settings is None:
        payload = {
            "schema_version": "okcanvas-step075-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "state": "FAILED",
            "started_at": _utc_now(),
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP075", output=output) as workspace:
            ...
            with TestClient(app) as client:
                ...
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": f"step075-live-{uuid.uuid4().hex}",
                    },
                )
