...
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP094_DETERMINISTIC_ACCEPTANCE.json"
...
def run(output: Path, *, emit_stdout: bool = True) -> int:
    ...
    checks = {
        ...
        "runtime_package_metadata_matches_baseline": 'version = "2.78.0"' in runtime_pyproject,
        ...
        "launcher_registry_current_exact": registry.get("state") == "PASSED" and registry.get("current_step") == STEP and registry.get("current_step_token") == "094" and registry.get("current_record_count") == 2,
        ...
        "package_identity_exact": PACKAGE_STEP == STEP and DEFAULT_OUTPUT.name == "okcanvas-agent-runtime-step094-cross-domain-stable-focus-and-groupware-context-filter.zip",
    }
    payload = {
        "schema_version": "okcanvas-step094-deterministic-acceptance-v1",
        "step": STEP, "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_STEP094_CROSS_DOMAIN_STABLE_FOCUS_AND_GROUPWARE_CONTEXT_FILTER",
