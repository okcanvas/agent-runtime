...
VERSION = "2.63.0"
RECORD = ROOT / "docs/evidence/STEP083_CONSTITUTION_COMPLIANCE.json"
...
def validate(path: Path = RECORD) -> dict[str, Any]:
    ...
    architecture = _load(ROOT / "docs/evidence/STEP083_ARCHITECTURE_VALIDATION.json")
    assistant = _load(ROOT / "docs/evidence/STEP083_ASSISTANT_ROUTING_VALIDATION.json")
    python_regression = _load(ROOT / "docs/evidence/STEP083_PYTHON_REGRESSION.json")
    non_python = _load(ROOT / "docs/evidence/STEP083_NON_PYTHON_VALIDATION.json")
    installation = _load(ROOT / "docs/evidence/STEP083_INSTALLATION_VALIDATION.json")
    fresh = _load(ROOT / "docs/evidence/STEP083_FRESH_ZIP_VALIDATION.json")
    acceptance = _load(ROOT / "docs/evidence/STEP083_ACCEPTANCE.json")
    ...
    checks = {
        ...
        "assistant_routing_passed": assistant.get("state") == "PASSED" and assistant.get("passed_checks") == assistant.get("total_checks") == 18,
        ...
        "issues_complete": all(f"OR-ISSUE-{number:03d}" in issue_registry for number in range(17, 71)),
        "windows_product_rerun_separate_from_constitution_gate": record.get("validation_environment", {}).get("step083_windows_deterministic") == "PENDING_EXTERNAL_PRODUCT_PROMOTION_GATE",
    }
    return {
        "schema_version": "okcanvas-step083-compliance-validation-v1",
        "step": STEP,
        "version": VERSION,
