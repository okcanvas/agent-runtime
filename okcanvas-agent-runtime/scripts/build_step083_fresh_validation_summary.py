...
def build(
    *,
    ...
    assistant_routing_path: Path,
    execution_plane_path: Path,
    distribution_path: Path,
    acceptance_path: Path,
    portability_path: Path,
    non_python_path: Path,
    ...
    evidence = {
        ...
        "assistant_routing": _load(assistant_routing_path),
        "execution_plane": _load(execution_plane_path),
        "distribution": _load(distribution_path),
        "acceptance": _load(acceptance_path),
        "portability": _load(portability_path),
        "non_python": _load(non_python_path),
        "installation": _load(installation_path),
        "python_regression": _load(python_regression_path),
    }
    with zipfile.ZipFile(candidate_zip) as archive:
        ...
        forbidden = sorted(
            ...
            or "/docs/evidence/step083-local/" in name
        )
    ...
    checks = {
        ...
        "assistant_routing_passed": evidence["assistant_routing"].get("state") == "PASSED"
        and evidence["assistant_routing"].get("passed_checks") == evidence["assistant_routing"].get("total_checks") == 18,
        ...
        and evidence["acceptance"].get("passed_checks") == evidence["acceptance"].get("total_checks") == 12,
        ...
        and evidence["python_regression"].get("passed_tests") == evidence["python_regression"].get("total_tests")
    ...
    payload = {
        "schema_version": "okcanvas-step083-fresh-zip-validation-v1",
        "step": STEP,
        "version": VERSION,
...
def main() -> int:
    ...
    parser.add_argument("--assistant-routing", type=Path, required=True)
    parser.add_argument("--execution-plane", type=Path, required=True)
    parser.add_argument("--distribution", type=Path, required=True)
    parser.add_argument("--acceptance", type=Path, required=True)
    parser.add_argument("--portability", type=Path, required=True)
    parser.add_argument("--non-python", type=Path, required=True)
    parser.add_argument("--installation", type=Path, required=True)
    parser.add_argument("--python-regression", type=Path, required=True)
    ...
    payload = build(
        ...
        assistant_routing_path=args.assistant_routing.resolve(),
        execution_plane_path=args.execution_plane.resolve(),
        distribution_path=args.distribution.resolve(),
        acceptance_path=args.acceptance.resolve(),
        portability_path=args.portability.resolve(),
        non_python_path=args.non_python.resolve(),
        installation_path=args.installation.resolve(),
        python_regression_path=args.python_regression.resolve(),
