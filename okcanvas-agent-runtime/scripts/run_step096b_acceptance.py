...
from scripts.validate_step096b_static_contract import STEP, VERSION, validate

OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP096B_DETERMINISTIC_ACCEPTANCE.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) > 1:
        raise SystemExit("run_step096b_acceptance accepts at most one output path")
    ...
    checks = {
        "step096b_static_contract": static["state"] == "PASSED",
        "python_compileall": compile_result.returncode == 0,
        "focused_regression_pytest": pytest_result.returncode == 0,
        "focused_regression_exact_pass_count": "63 passed" in summary,
        "windows_live_not_claimed": True,
        "broad_historical_suite_not_claimed": True,
    }
    payload = {
        "schema_version": "okcanvas-step096b-deterministic-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_GROUNDED_LLM_STRUCTURED_DELEGATION_ADMISSION",
