...
STEP = "STEP090_ORGANIZATION_CONTEXT_AMBIGUOUS_RESULT_DETERMINISTIC_NORMALIZATION_AND_LIVE_DIAGNOSTIC_CLOSURE"
VERSION = "2.70.0"
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP090_DETERMINISTIC_ACCEPTANCE.json"
PARENT_PATH = ROOT / "docs/evidence/STEP089_DETERMINISTIC_ACCEPTANCE.json"
EXPECTED_PACKAGE_NAME = (
    "okcanvas-agent-runtime-step090-organization-context-ambiguous-result-deterministic-"
    "normalization-and-live-diagnostic-closure.zip"
)
...
def run(output: Path, *, emit_stdout: bool = True) -> int:
    ...
    print("[STEP090] execution-plane", file=sys.stderr, flush=True)
    execution = validate_execution_plane()
    print("[STEP090] distribution", file=sys.stderr, flush=True)
    distribution = validate_distribution()
    print("[STEP090] launcher-registry", file=sys.stderr, flush=True)
    registry = validate_registry()
    print("[STEP090] architecture", file=sys.stderr, flush=True)
    ...
    print("[STEP090] focused-regression", file=sys.stderr, flush=True)
    ...
    print("[STEP090] compileall", file=sys.stderr, flush=True)
    ...
    checks = {
        ...
        "step089_parent_retained": parent.get("state") == "PASSED"
        and parent.get("passed_checks") == parent.get("total_checks") == 23,
    ...
    payload = {
        "schema_version": "okcanvas-step090-deterministic-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        ...
        "limitations": {
            ...
            "windows_step090_executed": False,
        },
    }
