...
ADMIN_KEY = "step078-live-admin-key-123456789"
SUBMITTER_KEY = "step078-live-submitter-key-123456"
SERVICE_TOKEN = "step078-live-service-token-123456789"
...
def _default_output() -> Path:
    ...
    return ROOT / "docs/evidence/step078-live" / f"{stamp}-{uuid.uuid4().hex[:8]}" / "acceptance-summary.json"
    ...
    return json.dumps(
        {
            ...
            "tokens": [
                {
                    "token_id": "step078-live-client",
                    "token_sha256": _sha(SERVICE_TOKEN),
                    "tenant_id": "step078-tenant",
                    "principal_id": "step078-reviewer",
                    "roles": ["agent-user"],
                }
            ],
        },
...
def _wait_terminal(client: TestClient, run_id: str, timeout: float = 300.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if payload.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return payload
        time.sleep(0.25)
    raise RuntimeError("STEP078 live Run did not become terminal within 300 seconds")
...
def run_acceptance(output: Path) -> int:
    ...
    if settings is None:
        payload = {
            "schema_version": "okcanvas-step078-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "state": "FAILED",
            "started_at": _utc_now(),
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP078", output=output) as workspace:
            ...
            with TestClient(app) as client:
                ...
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": f"step078-live-{uuid.uuid4().hex}",
                    },
                )
            ...
            checks = {
                ...
                and submission_owner.tenant_id == "step078-tenant"
                and submission_owner.principal_id == "step078-reviewer",
                ...
                and runtime_info.product_owned_atomic_service_submission_ownership_transfer_windows_live_accepted is False,
                ...
                and runtime_info.next_selected_step == "UNSELECTED_PENDING_STEP078_WINDOWS_LIVE_ACCEPTANCE",
            }
