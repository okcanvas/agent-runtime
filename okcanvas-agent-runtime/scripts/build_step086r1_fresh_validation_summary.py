...
def build(*, candidate_zip: Path, extracted_root: Path, fresh_python_regression: Path, fresh_acceptance: Path, output: Path) -> dict[str, Any]:
    ...
    evidence = {
        ...
        "architecture": _load(evidence_root / "STEP086R1_ARCHITECTURE_VALIDATION.json"),
        "groupware": _load(evidence_root / "STEP086R1_GROUPWARE_BOUNDARY_VALIDATION.json"),
        "execution_plane": _load(evidence_root / "STEP086R1_EXECUTION_PLANE_VALIDATION.json"),
        "distribution": _load(evidence_root / "STEP086R1_DISTRIBUTION_VALIDATION.json"),
        "acceptance": _load(evidence_root / "STEP086R1_ACCEPTANCE.json"),
        "portability": _load(evidence_root / "STEP086R1_WINDOWS_SUBPROCESS_PORTABILITY_VALIDATION.json"),
        "non_python": _load(evidence_root / "STEP086R1_NON_PYTHON_VALIDATION.json"),
        "installation": _load(evidence_root / "STEP086R1_INSTALLATION_VALIDATION.json"),
        "python_regression": _load(evidence_root / "STEP086R1_PYTHON_REGRESSION.json"),
        "compliance": _load(evidence_root / "STEP086R1_COMPLIANCE_VALIDATION.json"),
    }
    with zipfile.ZipFile(candidate_zip) as archive:
        ...
        forbidden = sorted(
            ...
            or "/docs/evidence/step086r1-local/" in name
        )
    ...
    checks = {
        ...
        and evidence["groupware"].get("passed_checks") == evidence["groupware"].get("total_checks") == 25,
        ...
        and evidence["acceptance"].get("passed_checks") == evidence["acceptance"].get("total_checks") == 13,
        ...
        and evidence["python_regression"].get("passed_tests") == evidence["python_regression"].get("total_tests") == 973
        ...
        and evidence["fresh_python_regression"].get("passed_tests") == evidence["fresh_python_regression"].get("total_tests") == 973
        ...
        and evidence["fresh_acceptance"].get("passed_checks") == evidence["fresh_acceptance"].get("total_checks") == 13,
    }
    payload = {
        "schema_version": "okcanvas-step086r1-final-fresh-validation-v1",
        "step": STEP,
        "version": VERSION,
