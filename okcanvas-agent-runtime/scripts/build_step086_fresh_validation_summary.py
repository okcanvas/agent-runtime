...
def build(*, candidate_zip: Path, extracted_root: Path, fresh_python_regression: Path, fresh_acceptance: Path, output: Path) -> dict[str, Any]:
    ...
    evidence = {
        ...
        "architecture": _load(evidence_root / "STEP086_ARCHITECTURE_VALIDATION.json"),
        "groupware": _load(evidence_root / "STEP086_GROUPWARE_READ_ONLY_VALIDATION.json"),
        "execution_plane": _load(evidence_root / "STEP086_EXECUTION_PLANE_VALIDATION.json"),
        "distribution": _load(evidence_root / "STEP086_DISTRIBUTION_VALIDATION.json"),
        "acceptance": _load(evidence_root / "STEP086_ACCEPTANCE.json"),
        "portability": _load(evidence_root / "STEP086_WINDOWS_SUBPROCESS_PORTABILITY_VALIDATION.json"),
        "non_python": _load(evidence_root / "STEP086_NON_PYTHON_VALIDATION.json"),
        "installation": _load(evidence_root / "STEP086_INSTALLATION_VALIDATION.json"),
        "python_regression": _load(evidence_root / "STEP086_PYTHON_REGRESSION.json"),
        "compliance": _load(evidence_root / "STEP086_COMPLIANCE_VALIDATION.json"),
    }
    with zipfile.ZipFile(candidate_zip) as archive:
        ...
        forbidden = sorted(
            ...
            or "/docs/evidence/step086-local/" in name
        )
    ...
    checks = {
        ...
        "groupware_passed": evidence["groupware"].get("state") == "PASSED"
        and evidence["groupware"].get("passed_checks") == evidence["groupware"].get("total_checks") == 30,
        ...
        and evidence["acceptance"].get("passed_checks") == evidence["acceptance"].get("total_checks") == 14,
        ...
        and evidence["installation"].get("wheel_payload_file_count") == 350,
        "python_regression_passed": evidence["python_regression"].get("state") == "PASSED"
        and evidence["python_regression"].get("passed_tests") == evidence["python_regression"].get("total_tests") == 962
        ...
        and evidence["fresh_python_regression"].get("passed_tests") == evidence["fresh_python_regression"].get("total_tests") == 962
        ...
        and evidence["fresh_acceptance"].get("passed_checks") == evidence["fresh_acceptance"].get("total_checks") == 14,
    }
    payload = {
        "schema_version": "okcanvas-step086-final-fresh-validation-v1",
        "step": STEP,
        "version": VERSION,
