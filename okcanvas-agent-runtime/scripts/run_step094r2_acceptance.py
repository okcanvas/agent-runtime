...
OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP094R2_DETERMINISTIC_ACCEPTANCE.json"
...
def run(output: Path, *, emit_stdout: bool = True) -> int:
    ...
    focused_ok, focused_output = _run([
        ...
        "tests/test_step094r2_cross_domain_submission_admission_owner.py",
    ...
    checks = {
        ...
        "runtime_package_metadata_matches_baseline": 'version = "2.78.2"' in runtime_pyproject,
        ...
        "launcher_registry_current_exact": registry.get("state") == "PASSED" and registry.get("current_step") == STEP and registry.get("current_step_token") == "094R2" and registry.get("current_record_count") == 2,
        ...
        "package_identity_exact": PACKAGE_STEP == STEP and DEFAULT_OUTPUT.name == "okcanvas-agent-runtime-step094r2-cross-domain-run-submission-admission-owner-closure.zip",
    }
    payload = {
        "schema_version": "okcanvas-step094-deterministic-acceptance-v1",
        "step": STEP, "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_STEP094R2_CROSS_DOMAIN_RUN_SUBMISSION_ADMISSION_OWNER_CLOSURE",
        ...
        "limitations": {
            "tests_deferred_by_user_until_minio_ready": True,
