...
DEFAULT_OUTPUT = ROOT / "docs/evidence/step072b-live/STEP072B_LIVE_ACCEPTANCE.json"
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise TypeError("STEP072A child evidence must be an object")
    return value


def run(output: Path) -> int:
    child_output = output.parent / "STEP072A_CHILD_LIVE_ACCEPTANCE.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    if child_output.exists():
        child_output.unlink()

    api_key = os.getenv("OPENAI_API_KEY", "")
    model = os.getenv("OKCANVAS_AGENT_MODEL", "").strip()
    ...
    checks = {
        "live_environment_ready": bool(api_key.strip()) and model == EXPECTED_MODEL,
        "local_environment_forwarded_to_current_interpreter": bool(api_key.strip())
        ...
        "step072a_child_process_succeeded": completed.returncode == 0,
        "step072a_child_evidence_loaded": child_error is None and bool(child),
        "step072a_live_checks_exact": child.get("state") == "PASSED"
        and child.get("passed_checks") == 15
        and child.get("total_checks") == 15,
        ...
        "api_key_not_in_captured_output": bool(api_key) and api_key not in combined,
        "child_live_environment_ready": child_checks.get("live_environment_ready") is True,
    ...
    payload: dict[str, Any] = {
        "schema_version": "okcanvas-step072b-live-acceptance-v1",
        "step": STEP,
        "version": VERSION,
