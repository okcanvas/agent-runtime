...
VERSION = "2.62.2"
RECORD = ROOT / "docs/evidence/STEP082B_CONSTITUTION_COMPLIANCE.json"
...
def validate(path: Path = RECORD) -> dict[str, Any]:
    ...
    architecture = _load(ROOT / "docs/evidence/STEP082B_ARCHITECTURE_VALIDATION.json")
    python_regression = _load(ROOT / "docs/evidence/STEP082B_PYTHON_REGRESSION.json")
    non_python = _load(ROOT / "docs/evidence/STEP082B_NON_PYTHON_VALIDATION.json")
    installation = _load(ROOT / "docs/evidence/STEP082B_INSTALLATION_VALIDATION.json")
    fresh = _load(ROOT / "docs/evidence/STEP082B_FRESH_ZIP_VALIDATION.json")
    acceptance = _load(ROOT / "docs/evidence/STEP082B_ACCEPTANCE.json")
    ...
    checks = {
        ...
        "full_python_passed": python_regression.get("state") == "PASSED" and python_regression.get("passed_tests") == python_regression.get("total_tests") and python_regression.get("test_file_count") == 231,
        ...
        "issues_complete": all(f"OR-ISSUE-{number:03d}" in issue_registry for number in range(17, 62)),
        "windows_product_rerun_separate_from_constitution_gate": record.get("validation_environment", {}).get("step082b_windows_deterministic") == "PENDING_EXTERNAL_PRODUCT_PROMOTION_GATE",
    }
    return {
        "schema_version": "okcanvas-step082b-compliance-validation-v1",
        "step": STEP,
        "version": VERSION,
