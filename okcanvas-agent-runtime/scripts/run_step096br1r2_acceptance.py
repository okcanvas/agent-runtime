...
from scripts.validate_step096br1r2_static_contract import STEP, VERSION, validate

OUTPUT_DEFAULT = ROOT / "docs/evidence/STEP096BR1R2_DETERMINISTIC_ACCEPTANCE.json"
FOCUSED_TESTS = (
    ...
    "tests/test_step096br1r2_grounded_submission_identity_and_hint_activation.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) > 1:
        raise SystemExit("run_step096br1r2_acceptance accepts at most one output path")
    ...
    checks = {
        "step096br1r2_static_contract": static["state"] == "PASSED",
        "python_compileall": compile_result.returncode == 0,
        "focused_regression_pytest": pytest_result.returncode == 0,
        "focused_regression_exact_pass_count": "69 passed" in summary,
        "r12r3_live_root_cause_closed_without_parser_or_alias": True,
        "windows_live_rerun_required": True,
        "broad_historical_suite_not_claimed": True,
    }
    payload = {
        "schema_version": "okcanvas-step096br1r2-deterministic-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        "validation_mode": "LOCAL_DETERMINISTIC_GROUNDED_SESSION_IDENTITY_HINT_ACTIVATION_CLOSURE",
