...
ADMIN_KEY = "step075f-live-admin-key-123456789"
SUBMITTER_KEY = "step075f-live-submitter-key-123456"
SERVICE_TOKEN = "step075f-live-service-token-123456789"
...
def _default_output() -> Path:
    ...
    return ROOT / "docs/evidence/step075f-live" / f"{stamp}-{uuid.uuid4().hex[:8]}" / "acceptance-summary.json"
...
def _wait_terminal(client: TestClient, run_id: str, timeout: float = 300.0) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        ...
        if payload.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return payload
        time.sleep(0.25)
    raise RuntimeError("STEP075F live Run did not become terminal within 300 seconds")
...
def run_acceptance(output: Path) -> int:
    ...
    if settings is None:
        payload = {
            "schema_version": "okcanvas-step075f-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "state": "FAILED",
            "started_at": _utc_now(),
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP075F", output=output) as workspace:
            ...
            with TestClient(app) as client:
                ...
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": f"step075f-live-{uuid.uuid4().hex}",
                    },
                )
            ...
            repair_started = [
                item for item in events
                if item.get("event_type") == "agent.output.repair.started"
            ]
            repair_completed = [
                item for item in events
                if item.get("event_type") == "agent.output.repair.completed"
            ]
            ...
            checks = {
                ...
                "bounded_model_calls_observed": model_started == model_completed
                and model_started in {2, 3},
                ...
                "bounded_answer_repair_consistent": len(repair_started) == len(repair_completed)
                and len(repair_started) in {0, 1}
                and (not repair_completed or repair_completed[0].get("payload", {}).get("complete") is True)
                and (not repair_completed or repair_completed[0].get("payload", {}).get("tool_reexecuted") is False),
                "answer_completeness_runtime_bound": runtime_info.product_owned_readonly_sandbox_answer_completeness_implemented is True
                and runtime_info.product_owned_readonly_sandbox_bounded_answer_repair_implemented is True
                and runtime_info.product_owned_readonly_sandbox_answer_repair_max_model_calls == 1
                and runtime_info.product_owned_readonly_sandbox_answer_repair_tool_reexecution_allowed is False
                and runtime_info.next_selected_step == "UNSELECTED_PENDING_STEP075F_WINDOWS_LIVE_ACCEPTANCE",
            }
            ...
            payload = {
                ...
                "answer_completeness": {
                    ...
                    "repair_started_count": len(repair_started),
                    "repair_completed_count": len(repair_completed),
                    "repair_applied": len(repair_completed) == 1,
