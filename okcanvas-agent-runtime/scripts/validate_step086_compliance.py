...
VERSION = "2.66.0"
RECORD = ROOT / "docs/evidence/STEP086_CONSTITUTION_COMPLIANCE.json"
...
def validate(path: Path = RECORD) -> dict[str, Any]:
    ...
    architecture = _load(ROOT / "docs/evidence/STEP086_ARCHITECTURE_VALIDATION.json")
    groupware = _load(ROOT / "docs/evidence/STEP086_GROUPWARE_READ_ONLY_VALIDATION.json")
    python_regression = _load(ROOT / "docs/evidence/STEP086_PYTHON_REGRESSION.json")
    non_python = _load(ROOT / "docs/evidence/STEP086_NON_PYTHON_VALIDATION.json")
    installation = _load(ROOT / "docs/evidence/STEP086_INSTALLATION_VALIDATION.json")
    portability = _load(ROOT / "docs/evidence/STEP086_WINDOWS_SUBPROCESS_PORTABILITY_VALIDATION.json")
    acceptance = _load(ROOT / "docs/evidence/STEP086_ACCEPTANCE.json")
    ...
    checks = {
        ...
        "groupware_passed": groupware.get("state") == "PASSED" and groupware.get("passed_checks") == groupware.get("total_checks") == 30,
        "full_python_passed": python_regression.get("state") == "PASSED" and python_regression.get("passed_tests") == python_regression.get("total_tests") == 962 and python_regression.get("test_file_count") == len(list((ROOT / "tests").glob("test_*.py"))),
        ...
        "acceptance_passed": acceptance.get("state") == "PASSED" and acceptance.get("passed_checks") == acceptance.get("total_checks") == 14,
        "issues_complete": all(f"OR-ISSUE-{number:03d}" in issue_registry for number in range(17, 92)),
        "windows_product_rerun_separate_from_constitution_gate": record.get("validation_environment", {}).get("step085_windows_deterministic") == "ACCEPTED_12_OF_12" and record.get("validation_environment", {}).get("step086_windows_deterministic") == "PENDING_EXTERNAL_PRODUCT_PROMOTION_GATE",
    }
    return {
        "schema_version": "okcanvas-step086-compliance-validation-v1",
        "step": STEP,
        "version": VERSION,
