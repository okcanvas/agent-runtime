...
ADMIN_KEY = "step054-local-admin-key"
SUBMITTER_KEY = "step054-run-submitter-key"
...
MODEL = "deterministic-step054-model"
REQUEST = "Return a safe final answer with provider-side Responses storage explicitly disabled."
HIDDEN_API_KEY = "step054-hidden-api-key"
REASONING_SUMMARY_SENTINEL = "STEP054-PRIVATE-REASONING-SUMMARY"
REASONING_CONTENT_SENTINEL = "STEP054-PRIVATE-REASONING-CONTENT"
REASONING_ENCRYPTED_SENTINEL = "STEP054-ENCRYPTED-REASONING"
REASONING_PROVIDER_SENTINEL = "STEP054-PROVIDER-DATA"
REASONING_ITEM_ID = "rs_step054_private"
...
def _install_fakes():
    ...
    output = CodingAgentResult(
        ...
        summary="Safe output completed with OpenAI Responses storage disabled.",
        findings=[],
        unverified=[],
    )
    ...
    class FakeResult:
        def __init__(self, *, agent, hooks):
            ...
            self.last_response_id = "resp-step054"
        ...
        async def stream_events(self):
            ...
            response = SimpleNamespace(
                response_id="resp-step054",
                request_id="req-step054",
    ...
    class FakeRunner:
        ...
        async def run(cls, *args, **kwargs):
            ...
            raise AssertionError("STEP054 must use Runner.run_streamed")
    ...
    fake_agents.gen_trace_id = lambda: "trace-step054"
...
def _wait_terminal(client: TestClient, run_id: str) -> dict[str, object]:
    ...
    while time.monotonic() < deadline:
        ...
        if body.get("status") in {"SUCCEEDED", "FAILED", "CANCELLED"}:
            return body
        time.sleep(0.02)
    raise RuntimeError("STEP054 Run did not become terminal")
...
def run_acceptance(output: Path) -> int:
    ...
    try:
        with AcceptanceWorkspace(step_id="STEP054", output=output) as workspace:
            ...
            with TestClient(app) as client:
                denied = client.post(
                    ...
                    json={
                        ...
                        "idempotency_key": "step054-denied-route-0001",
                    },
                )
                preflight_response = client.post(
                    ...
                    json={
                        ...
                        "input": REQUEST,
                        "model": MODEL,
                        "idempotency_key": "step054-success-0001",
                    },
                )
                ...
                evaluation_response = client.post(
                    ...
                    json={"case_id": "immutable-openai-response-storage-disabled-v1"},
                )
                ...
                drift_preflight_response = client.post(
                    ...
                    json={
                        "agent_definition_id": "coding-agent",
                        "input": "Response storage policy drift must block confirmation before Product state.",
                        "model": MODEL,
                        "idempotency_key": "step054-policy-drift-0001",
                    },
                )
                drift_preflight_response.raise_for_status()
                drift_preflight = drift_preflight_response.json()
                policy_path = project_root / "specs/runtime/openai-response-storage-policy.json"
            ...
            policy = ResponseStoragePolicyCatalog(project_root).resolve()
            ...
            checks = {
                ...
                "exact_response_storage_policy_bound": (
                    binding.response_storage_policy["policy_sha256"] == policy.policy_sha256
                    and len(binding.response_storage_runtime_sha256) == 64
                ),
                "responses_store_explicitly_disabled": (
                    captured["run_config_store"] == [False]
                    and policy.response_store_requested is False
                ),
                ...
                "model_started_storage_policy_metadata_exact": len(model_events) == 1
                and model_events[0]["payload"].get("response_storage_policy_id")
                == "local-openai-response-storage-disabled-v1"
                and model_events[0]["payload"].get("response_storage_policy_sha256")
                == policy.policy_sha256
                and model_events[0]["payload"].get("response_store_requested") is False,
                "model_completed_remains_safe_and_bounded": len(completed_events) == 1
                and completed_events[0]["payload"].get("output_item_count") == 2
                and completed_events[0]["payload"].get("reasoning_item_count") == 1
                and completed_events[0]["payload"].get("request_id_present") is True,
                "prior_reasoning_private_fields_absent_from_events": all(
                    value not in event_text for value in sensitive_values
                ),
                "prior_reasoning_private_fields_absent_from_product_and_evaluation_db": all(
                    value not in product_text and value not in evaluation_text
                    for value in sensitive_values
                ),
                "prior_reasoning_private_fields_absent_from_artifact": all(
                ...
                "response_storage_policy_drift_blocked_confirmation": drift_confirmation.status_code == 409,
                "response_storage_policy_drift_created_no_second_task_or_run": final_counts["tasks"] == 1
                ...
                "runtime_binding_contains_no_secret_or_endpoint": HIDDEN_API_KEY
            ...
            report = {
                "schema_version": "okcanvas-step054-acceptance-v1",
                "state": "PASSED" if all(checks.values()) else "FAILED",
                "checks": checks,
                "response_storage_policy": {
                    **policy.to_binding_dict(),
                    "runtime_binding_sha256": binding.runtime_binding_sha256,
                    "runtime_source_sha256": binding.response_storage_runtime_sha256,
                    "captured_model_settings_store": captured["run_config_store"],
                },
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "docs/evidence/STEP054_ACCEPTANCE.json",
    )
