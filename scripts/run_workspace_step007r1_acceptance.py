...
STEP = "WORKSPACE_STEP007R1_ORGANIZATION_CONTEXT_BOUNDED_RESPONSE_AND_LIVE_FAILURE_DIAGNOSTIC_CLOSURE"
VERSION = "0.7.1"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP007R1_ACCEPTANCE.json"
...
def failure(started: str, errors: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "okcanvas-agent-platform-workspace-step007r1-acceptance-v1",
        ...
        "limitations": {"runtime_organization_context_wired": True, "windows_step007r1_live_executed": False, "real_enterprise_organization_context_called": False, "openai_model_called": False, "production_database_executed": False},
    }
...
def run(output: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="s007r1-") as temp_name:
        temp = Path(temp_name)
        connector = temp / "c"
        example = temp / "e"
        ...
        for key in ("OPENAI_API_KEY", "OKCANVAS_AGENT_MODEL", "OKCANVAS_WORKSPACE_STEP007R1_LIVE_ACCEPTANCE", "OKCANVAS_LOCAL_ENV_SOURCE_NAME", "OKCANVAS_LOCAL_ENV_LOADED_KEYS"):
            live_env.pop(key, None)
        live_preflight = run_process(sys.executable, [str(ROOT / "scripts/run_workspace_step007r1_live_acceptance.py"), "--example-root", str(example), "--output", str(live_output)], cwd=ROOT, env=live_env)
    ...
    parent = json.loads((ROOT / "docs/evidence/WORKSPACE_STEP007_WINDOWS_ACCEPTANCE_SUMMARY.json").read_text(encoding="utf-8"))
    ...
    runtime = json.loads((runtime_root / "docs/evidence/STEP088R1_DETERMINISTIC_ACCEPTANCE.json").read_text(encoding="utf-8"))
    ...
    checks = {
        ...
        "step007_windows_parent_retained": parent.get("state") == "PASSED" and parent.get("passed_checks") == 30,
        "step004r2_windows_live_parent_retained": live_parent.get("state") == "PASSED" and live_parent.get("live", {}).get("passed_checks") == 22,
        "runtime_step088r1_identity_exact": projects.get("agent-runtime", {}).get("baseline") == "STEP088R1_ORGANIZATION_CONTEXT_BOUNDED_RESPONSE_AND_SAFE_MCP_DIAGNOSTIC_CLOSURE" and projects.get("agent-runtime", {}).get("version") == "2.68.1",
        "runtime_step088r1_acceptance_passed": runtime.get("state") == "PASSED" and runtime.get("passed_checks") == 24 and runtime.get("total_checks") == 24,
        ...
        "organization_context_remote_mcp_exact": server.get("server_id") == "organization-context-read" and server.get("credential_ref") == "organization-context-read-credential" and len(server.get("allowed_tools", [])) == 3 and int(server.get("max_result_chars") or 0) == 32000,
        "safe_mcp_failure_diagnostic_contract_exact": "MCP_RESULT_LIMIT_EXCEEDED" in runtime_source and "okcanvas-mcp-tool-failed-v1" in runtime_source and "raw_error_persisted" in runtime_source,
        ...
        "connector_acceptance_passed": connector_payload.get("state") == "PASSED" and connector_payload.get("passed_checks") == 11,
        "example_acceptance_passed": example_payload.get("state") == "PASSED" and example_payload.get("passed_checks") == 18,
        "connector_example_integration_passed": integration_payload.get("state") == "PASSED" and integration_payload.get("passed_checks") == 17,
        "bounded_exact_resolution_proven": integration_checks.get("bounded_exact_resolution_observed") is True,
        "compact_search_response_proven": integration_checks.get("compact_search_response_observed") is True,
        ...
        "live_harness_uses_actual_boundaries": all(token in (ROOT / "scripts/run_workspace_step007r1_live_acceptance.py").read_text(encoding="utf-8") for token in ("organization_context_mcp_server", "create_runtime_app", "src/cli.mjs", "organization-context-session-agent", "/api/v1/context/resolve")),
    ...
    payload = {
        ...
        "validation_mode": "LOCAL_DETERMINISTIC_BOUNDED_ORGANIZATION_CONTEXT_RESPONSE_AND_LIVE_DIAGNOSTIC_CLOSURE",
        ...
        "limitations": {"runtime_organization_context_wired": True, "local_step084_catalog_replaced": False, "windows_step007r1_live_executed": False, "real_enterprise_organization_context_called": False, "production_database_executed": False, "openai_model_called": False},
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    write_json_stdout(payload)
