...
STEP = "WORKSPACE_STEP005_ORGANIZATION_CONTEXT_CONNECTOR_AND_CONSTRUCTION_GUIDE_FOUNDATION"
VERSION = "0.5.0"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP005_ACCEPTANCE.json"
...
def failure(started: str, errors: list[str]) -> dict[str, Any]:
    return {
        "schema_version": "okcanvas-agent-platform-workspace-step005-acceptance-v1",
        "step": STEP,
        "version": VERSION,
...
def run(output: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="okcanvas-workspace-step005-") as temp_name:
        temp = Path(temp_name)
        connector = temp / "okcanvas-connectors/organization-context-mcp-server"
        example = temp / "okcanvas-connector-examples/organization-context/organization-context-api-fake"
        ...
        integration_env = {
            **connector_env,
            "OKCANVAS_ORGANIZATION_CONTEXT_EXAMPLE_ROOT": str(example),
        }
        integration = run_process(
            connector_python,
            ["scripts/run_optional_example_integration.py"],
            cwd=connector,
            env=integration_env,
        )
    ...
    checks = {
        ...
        "organization_context_connector_cataloged": any(
            ...
            and item.get("baseline") == "CONNECTOR_ORGANIZATION_CONTEXT_STEP001_READ_ONLY_MCP_AND_MUTABLE_API_BOUNDARY_FOUNDATION"
        ...
        "example_acceptance_passed": example_payload.get("state") == "PASSED" and example_payload.get("passed_checks") == 8,
