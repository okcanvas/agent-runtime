...
LIVE_GATE = "OKCANVAS_WORKSPACE_STEP008R4R12R1_GROUNDED_STRUCTURED_DELEGATION_LIVE_ACCEPTANCE"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP008R4R12R1_GROUNDED_STRUCTURED_DELEGATION_LIVE_ACCEPTANCE.json"
...
def _identity_provenance() -> tuple[dict[str, object], dict[str, bool]]:
    ...
    entrypoint_path = ROOT / "scripts/run_workspace_step008r4r12r1_grounded_structured_delegation_live_entrypoint.py"
    launcher_path = ROOT / "sh_run_workspace_step008r4r12r1_grounded_structured_delegation_live_acceptance.cmd"
...
async def execute(output: Path) -> dict[str, object]:
    ...
    if not all(preflight.values()):
        payload = {
            "schema_version": "okcanvas-workspace-step008r4r12r1-grounded-structured-delegation-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "runtime_step": RUNTIME_STEP,
            "runtime_version": RUNTIME_VERSION,
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(
        json.dumps(redact(payload, secrets), ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    return payload
