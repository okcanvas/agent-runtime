...
EXAMPLE_ROOT = ROOT / "okcanvas-connector-examples/groupware/groupware-api-fake"
...
VERSION = "0.4.2"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP004R2_LIVE_ACCEPTANCE.json"
LIVE_GATE = "OKCANVAS_WORKSPACE_STEP004_LIVE_ACCEPTANCE"
EXAMPLE_GROUPWARE_API_TOKEN = "example-groupware-api-token"
    ...
    def __init__(
        ...
        *,
        ...
        self.thread = threading.Thread(
            ...
            name=f"uvicorn-step004-{self.port}",
            daemon=True,
        )
...
def create_loopback_certificates(target: Path) -> tuple[Path, Path, Path]:
    ...
    ca_name = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, "OKCanvas STEP004 Loopback CA")])
    ca_cert = (
...
async def execute(example_root: Path, output: Path) -> dict[str, object]:
    ...
    if not all(preflight.values()):
        payload = {
            "schema_version": "okcanvas-agent-platform-workspace-step004-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
            "validation_mode": "WINDOWS_LIVE_OPENAI_FULL_PROCESS_E2E",
            "state": "FAILED",
            "started_at": started,
            "completed_at": now(),
            "checks": preflight,
    ...
    external_token = random_secret("step004-service")
    connector_token = random_secret("step004-connector")
    groupware_token = EXAMPLE_GROUPWARE_API_TOKEN
    admin_key = random_secret("step004-admin")
    submitter_key = random_secret("step004-submitter")
    ...
    secrets = [api_key, external_token, connector_token, groupware_token, admin_key, submitter_key, payload_key, session_key]
    ...
    temp = Path(tempfile.mkdtemp(prefix="okcanvas-workspace-step004r1-live-"))
    ...
    try:
        ...
        if setup["returncode"] != 0:
            raise RuntimeError("Node example dependency setup failed")
        build = run_command(npm, ["run", "build"], cwd=example, env=environment, secrets=secrets)
        if build["returncode"] != 0:
            raise RuntimeError("Node example build failed")
        ...
        connector_server = LiveASGIServer(
            connector_app,
        ...
        groupware_server["url_template"] = f"{connector_server.base_url}/tenants/{{tenant_id}}/mcp"
        ...
        prompts.write_text(
            "그룹웨어 공지에서 maintenance를 검색하고 확인된 공지 제목과 근거를 알려줘.\n"
        ...
        runs = evidence["runs"]
        if len(runs) != 2:
            raise RuntimeError("Live CLI did not create exactly two Runtime runs")
        first, second = runs
        first_types = event_types(first)
        second_types = event_types(second)
        ...
        captured = fake_items[0] if fake_items else {}
        first_content = first["artifact"]["content"]
        second_content = second["artifact"]["content"]
        session = evidence["sessions"][0] if evidence["sessions"] else {}
        session_item_count = int(session.get("item_count") or 0)
        public_surface = json.dumps(
            {"cli": cli, "evidence": evidence, "fake_requests": fake_requests},
            ensure_ascii=False,
            default=str,
        )
        checks = {
            **preflight,
            "cli_completed_two_prompts": cli["returncode"] == 0
            and "2개 요청 완료" in str(cli["stdout"]),
            "both_runtime_runs_succeeded": all(item["run"]["status"] == "SUCCEEDED" for item in runs),
            "actual_openai_model_events_observed": len(first_model_started) >= 3
            and len(first_model_completed) >= 3
            and len(second_model_started) >= 1
            and len(second_model_completed) >= 1,
            "configured_model_used": all(
                payload.get("model") == model
                for payload in [*first_model_started, *second_model_started]
            ),
            "root_agent_tool_called_once_first_turn": first_types.count("agent.tool.started") == 1
            and first_types.count("agent.tool.completed") == 1,
            "child_mcp_called_once_first_turn": first_types.count("tool.started") == 1
            and first_types.count("tool.completed") == 1,
            "continuation_skipped_child_and_mcp": "agent.tool.started" not in second_types
            and "tool.started" not in second_types,
            "actual_connector_and_node_example_reached": len(fake_items) == 1
            and captured.get("path") == "/api/v1/notices/search",
            "delegated_identity_forwarded": captured.get("tenant_id") == "tenant-a"
            and captured.get("principal_id") == "user-001"
            and captured.get("roles") == ["agent-user"]
            and bool(captured.get("delegation_id")),
            "groupware_api_bearer_redacted": captured.get("authorization_present") is True
            and captured.get("authorization_value_recorded") is False,
            "enterprise_citation_persisted": first_content.get("request_class") == "READ_SYSTEM"
            and first_content.get("side_effect") == "READ"
            and any(item.get("reference") == "notice-001" for item in first_content.get("citations", [])),
            "root_session_continuity_committed": session.get("turn_count") == 2
            and session_item_count >= 4
            and session_item_count % 2 == 0
            and "Maintenance notice" in str(second_content.get("answer", "")),
            "provider_identifiers_not_persisted": all(
                payload.get("provider_response_id_persisted") is False
                and payload.get("provider_request_id_persisted") is False
                for payload in [*first_model_started, *second_model_started]
            ),
            "response_storage_and_trace_export_remain_disabled": all(
                payload.get("response_store_requested") is False
                for payload in [*first_model_started, *second_model_started]
            ),
            "secrets_absent_from_evidence": all(secret not in public_surface for secret in secrets),
            "command_bearer_redacted": "[REDACTED]" in cli["command"]
            and external_token not in cli["command"],
        }
        payload = {
            "schema_version": "okcanvas-agent-platform-workspace-step004-live-acceptance-v1",
            "step": STEP,
            ...
            "cli": cli,
            "runtime": {
                "run_ids": [item["run"]["run_id"] for item in runs],
                "session_id": session.get("session_id"),
                "first_turn_model_request_count": len(first_model_started),
                "second_turn_model_request_count": len(second_model_started),
                "first_turn_invocation_count": len(first["invocations"]),
                "second_turn_invocation_count": len(second["invocations"]),
            },
            "connector_example": {
                "request_count": len(fake_items),
                "authorization_value_recorded": captured.get("authorization_value_recorded"),
            },
            "limitations": {
    ...
    payload["harness_cleanup"] = {
        "completed": cleanup_completed,
        "error_types": sorted(set(cleanup_error_types)),
        "transient_removal_error_types": sorted(set(transient_removal_error_types)),
        "processes_stopped_before_temp_removal": True,
    }
