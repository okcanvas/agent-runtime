...
VERSION = "0.7.1"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP007R1_LIVE_ACCEPTANCE.json"
LIVE_GATE = "OKCANVAS_WORKSPACE_STEP007R1_LIVE_ACCEPTANCE"
    ...
    def __init__(
        ...
        *,
        ...
        self.thread = threading.Thread(
            ...
            name=f"uvicorn-step007r1-{self.port}",
            daemon=True,
        )
...
async def execute(example_root: Path, output: Path) -> dict[str, object]:
    ...
    if not all(preflight.values()):
        payload = {
            "schema_version": "okcanvas-agent-platform-workspace-step007r1-live-acceptance-v1",
            "step": STEP,
            "version": VERSION,
    ...
    external_token = random_secret("step007r1-service")
    connector_token = random_secret("step007r1-connector")
    product_token = EXAMPLE_ORGANIZATION_CONTEXT_API_TOKEN
    admin_key = random_secret("step007r1-admin")
    submitter_key = random_secret("step007r1-submitter")
    ...
    temp = Path(tempfile.mkdtemp(prefix="okcanvas-workspace-step007r1-live-"))
    ...
    try:
        ...
        first, second = runs
        allowed_failure_keys = {"payload_schema_version", "failure_stage", "failure_category", "server_id", "tool_name", "observed_chars", "max_result_chars", "tool_arguments_persisted", "tool_result_persisted", "raw_error_persisted", "retryable"}
        safe_tool_failures = [
            {key: value for key, value in payload.items() if key in allowed_failure_keys}
            for item in runs
            for payload in event_payloads(item, "tool.failed")
        ]
