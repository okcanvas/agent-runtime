...
from workspace_inventory import (
    excluded_package_path,
    excluded_parent_project_path,
    excluded_workspace_path,
    snapshot_files,
)
...
STEP = "WORKSPACE_STEP003R2_WINDOWS_PROJECT_PYTHON_ENVIRONMENT_AND_REDIRECTED_LOG_IDENTITY_CLOSURE"
VERSION = "0.3.2"
OUTPUT_DEFAULT = ROOT / "docs/evidence/WORKSPACE_STEP003R2_ACCEPTANCE.json"
...
def manifest_drift() -> dict[str, list[str]]:
    ...
    return {"missing": missing, "changed": changed, "unexpected": unexpected}
...
def failure(started: str, errors: list[str]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "okcanvas-agent-platform-workspace-step003r2-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        "validation_mode": "DETERMINISTIC_FULL_PROCESS_BOUNDARY",
        "state": "FAILED",
        "started_at": started,
        "completed_at": now(),
        ...
        "errors": errors,
        "limitations": {
            "windows_step003r2_executed": os.name == "nt",
            "windows_step003r2_accepted": False,
...
def run(output: Path) -> int:
    ...
    parent_json_stdout_cp949_safe = (
        cp949_console_escaped
        and cp949_console_encoding == "ascii-json-escape"
        and json.loads(cp949_console_text) == console_probe
        and cp949_console_text.encode("cp949").decode("cp949") == cp949_console_text
    )
    ...
    with tempfile.TemporaryDirectory(prefix="okcanvas-workspace-step003r2-acceptance-") as temp_name:
        temp = Path(temp_name)
        ...
        connector_acceptance = run_process(
            connector_python, ["scripts/run_acceptance.py"], cwd=connector, env=connector_env
        )
    ...
    windows_parent = json.loads(
        (ROOT / "docs/evidence/WORKSPACE_STEP002R1_WINDOWS_ACCEPTANCE_SUMMARY.json").read_text(encoding="utf-8")
    )
    ...
    runtime_venv = venv_python(project_roots["runtime"])
    connector_venv = venv_python(project_roots["connector"])
    project_python_resolution_exact = (
        (runtime_venv is None or Path(runtime_python).resolve() == runtime_venv)
        and (connector_venv is None or Path(connector_python).resolve() == connector_venv)
    )

    platform = "windows" if os.name == "nt" else os.name
    checks = {
        "workspace_root_contract_exact": True,
        "workspace_manifest_exact": not any(drift.values()),
        "redirected_root_log_excluded_from_identity": excluded_workspace_path(Path("log.txt"))
        and excluded_package_path(Path("log.txt"))
        and excluded_workspace_path(Path("workspace-step003r2.log")),
        "external_node_tools_resolved": Path(node).is_absolute() and Path(npm).is_absolute(),
        "project_python_environments_resolved": Path(runtime_python).is_absolute()
        and Path(connector_python).is_absolute()
        and project_python_resolution_exact,
        "subprocess_output_encoding_safe": output_encoding_safe,
        "parent_json_stdout_cp949_safe": parent_json_stdout_cp949_safe,
        "workspace_unit_tests_passed": unit["returncode"] == 0,
        "runtime_step087_acceptance_passed": parsed["runtime"] is not None
        ...
        and parsed["cli"] is not None
        and parsed["cli"].get("state") == "PASSED"
        ...
        and parsed["connector"] is not None
        and parsed["connector"].get("state") == "PASSED"
        ...
        and parsed["example"] is not None
        and parsed["example"].get("state") == "PASSED"
        ...
        and parsed["connector_example"] is not None
        and parsed["connector_example"].get("state") == "PASSED"
        ...
        "runtime_step087_windows_not_claimed": parsed["runtime"] is not None
        and parsed["runtime"].get("limitations", {}).get("windows_step087_executed") is False,
        "execution_platform_recorded": platform in {"windows", "posix"},
        ...
        "step003r2_launcher_present": (ROOT / "sh_run_workspace_step003r2_acceptance.cmd").is_file(),
        "e2e_command_bearer_redacted": parsed["full_e2e"] is not None
        and "[REDACTED]" in e2e_text
        and "step003-external-service-bearer-123456" not in e2e_text,
    ...
    payload = {
        ...
        "resolved_executables": {
            "workspace_bootstrap_python": sys.executable,
        ...
        "project_environment_resolution": {
            "runtime_venv_present": runtime_venv is not None,
            "runtime_venv": str(runtime_venv) if runtime_venv else None,
            "runtime_selected": runtime_python,
            "connector_venv_present": connector_venv is not None,
            "connector_venv": str(connector_venv) if connector_venv else None,
            "connector_selected": connector_python,
        },
        "processes": {
        ...
        "limitations": {
            "windows_step003r2_executed": os.name == "nt",
            "windows_step003r2_accepted": os.name == "nt" and state == "PASSED",
            "live_openai_model_called": False,
            "real_enterprise_groupware_provider_called": False,
