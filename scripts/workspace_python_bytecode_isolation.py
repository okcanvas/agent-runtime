...
SCHEMA_VERSION = "okcanvas-workspace-python-bytecode-isolation-v1"
...
def build_isolated_environment(
    ...
    *,
    ...
    prefix = Path(tempfile.mkdtemp(prefix="okcanvas-workspace-pycache-", dir=str(parent))).resolve()
...
def run_isolated_python(arguments: list[str]) -> int:
    ...
    try:
        ...
        raise ValueError("The target script must be inside the Workspace root") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="workspace_python_bytecode_isolation")
    parser.add_argument("arguments", nargs=argparse.REMAINDER)
    args = parser.parse_args(argv)
    try:
        return run_isolated_python(list(args.arguments))
    except (OSError, ValueError) as exc:
        print(f"[ERROR] Workspace Python bytecode isolation failed: {exc}", file=sys.stderr)
        return 2
