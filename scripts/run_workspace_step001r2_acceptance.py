...
STEP = "WORKSPACE_STEP001R2_WINDOWS_SUBPROCESS_OUTPUT_ENCODING_CLOSURE"
VERSION = "0.1.2"
OUTPUT = ROOT / "docs/evidence/WORKSPACE_STEP001R2_ACCEPTANCE.json"
...
def failure_payload(started: str, errors: list[str]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "okcanvas-agent-platform-workspace-step001r2-acceptance-v1",
        "step": STEP,
        "version": VERSION,
        "state": "FAILED",
        "started_at": started,
...
def main() -> int:
    ...
    unit = run_process(sys.executable, ["-m", "unittest", "discover", "-s", "tests", "-v"], cwd=ROOT)
    ...
    checks = {
        ...
        "product_cli_foundation_acceptance_passed": cli_acceptance["returncode"] == 0
        and cli_payload is not None
        and cli_payload.get("state") == "PASSED"
        and cli_payload.get("passed_checks") == cli_payload.get("total_checks") == 6,
        ...
        "subproject_acceptance_did_not_mutate_source": unit["returncode"] == 0,
    }
    ...
    checks["subproject_acceptance_did_not_mutate_source"] = post_identity["returncode"] == 0
